...
def initialize_params(n):
    ...
    W1 = tf.compat.v1.get_variable("W1", [12*n,12*n], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    # initializes b as zeros as a 12 x 1 vector
    b1 = tf.compat.v1.get_variable("b1", [12*n,1], initializer = tf.zeros_initializer())
...
def optimize_params(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, n, learning_rate, num_epochs, minibatch_size):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # Finds the estimated output of the test set
        Y_hat_test = 1/(1+np.exp(-np.add(np.matmul(W1_np,X_test),b1_np)))
