...
def initialize_params():
    ...
    W1 = tf.compat.v1.get_variable("W1", [12,12], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
...
def optimize_params(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, learning_rate, num_epochs, minibatch_size):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for epoch in range(num_epochs): # loops over epochs
            ...
            a = np.square(np.add(np.matmul(W1_np,X_train),b1_np)-Y_train)
            train_costs.append(a[0:6,:].mean())
            
            b = np.square(np.add(np.matmul(W1_np,X_dev),b1_np)-Y_dev)
            dev_costs.append(b[0:6,:].mean())
                
        # plots training set and dev set costs
        plt.plot(np.squeeze(train_costs), label='Training Set Error')
        plt.plot(np.squeeze(dev_costs), label='Dev Set Error')
        plt.plot(0.07853173572682377*np.ones(len(train_costs)), label='Guessing Error')
        plt.legend()
        plt.ylabel('mean squared error')
        plt.ylim([0,max(train_costs)+0.01])
        plt.xlabel('Epochs')
        ...
        W1_np = parameters['W1'].eval(session=sess)
        b1_np = parameters['b1'].eval(session=sess)           
        c = np.add(np.matmul(W1_np,X_test),b1_np)        
        print('Test Set Error Mean: ',np.square(c[0:6,:]-Y_test[0:6,:]).mean(axis=0).mean(axis=0))
        print('Test Set Error Standard Deviation: ',np.square(c[0:6,:]-Y_test[0:6,:]).mean(axis=0).std())
